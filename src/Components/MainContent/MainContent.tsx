...
import { QUERY_KEYS, QUERY_KEYS_ADMIN_BASIC_INFO, QUERY_KEYS_STUDENT } from '../../utils/const';
...
function MainContent() {
    ...
    const [overallProfilePercentage, setoverallProfilePercentage] = useState<number>(0);
    const [desctiptionPercentage, setdesctiptionPercentage] = useState<number>(0)
    const [profileImage, setprofileImage] = useState<any>();
    const [dataComplated, setDatacomplated] = useState(false)
    ...
    const barData = {
        labels: ['Entities', 'Institute', 'Student', 'Course', 'Subject', 'Department'],
        datasets: [{
            label: 'Dataset 1',
            backgroundColor: '#3498DB',
            borderColor: '#3498DB',
            borderWidth: 1,
            data: [stats.entityCount, stats.institutionCount, stats.studentCount, stats.courseCount, stats.subjectCount, stats.departmentCount],
        }]
    };
    const lineData = {
        labels: ['Entities', 'Institute', 'Student', 'Course', 'Subject', 'Department'],
              ...
              .then((data: any) => {
                ...
                //   let basic_info = data.data.basic_info;
                  ...
                  let academic_history = {
                    institution_id:data?.data?.academic_history?.institution_id,
                    course_id:data?.data?.academic_history?.course_id,
                ...
                //   let contact = data.data.contact;
                  ...
                  let subject_preference = {
                    course_id:data?.data?.subject_preference?.course_id,
                    subject_id:data?.data?.subject_preference?.subject_id,
                    preference:data?.data?.subject_preference?.preference,
                    score_in_percentage:data?.data?.subject_preference?.score_in_percentage,
                };
                //   let hobby = data.data.hobby;
                  ...
                  if (sectionCount > 0) {
                    ...
                    localStorage.setItem('Profile_completion',JSON.stringify(overallPercentage));
    ...
    const fetchStudentData = async () => {
        ...
        try {
            ...
            const chatstarred = chatHistory?.data?.filter((chat:any)=>chat?.flagged) || []
            setStudent({
    ...
    const callAPIAdmin = async () => {
        ...
        getData(`${profileURLadmin}/${StudentId}`).then((data: any) => {
          if (data?.data) {
    ...
    useEffect(() => {
       if(userName !== 'admin') {
        ...
        const fetchData = async () => {
            ...
            try {
                const [institutionRes, studentRes, subjectRes, entityRes, departmentRes, courseRes] = await Promise.allSettled([
                ...
                const institutionCount = institutionRes?.status === 'fulfilled' ? institutionRes?.value?.data?.length || 0 : 0;
                const studentCount = studentRes?.status === 'fulfilled' ? studentRes?.value?.data?.length || 0 : 0;
                const subjectCount = subjectRes?.status === 'fulfilled' ? subjectRes?.value?.data?.length || 0 : 0;
                const entityCount = entityRes?.status === 'fulfilled' ? entityRes?.value?.data?.length || 0 : 0;
                const departmentCount = departmentRes?.status === 'fulfilled' ? departmentRes?.value?.data?.length || 0 : 0;
                const courseCount = courseRes?.status === 'fulfilled' ? courseRes?.value?.data?.length || 0 : 0;
        ...
        <section className="stats">
            {
                ...
                <Link to={EntityExists ? "/main/entity" : "#"} className="stat-item">
                {/* <Link to="/main/entity" className="stat-item"> */}
                    <div >
                        <p>Entities</p>
                        <h2>{stats.entityCount}</h2>
                    </div>
                </Link>
                <Link to={InstitutionsExists ? "/main/Institute" : "#" }className="stat-item">
                    <div >
                        <p>Institutions</p>
                        <h2>{stats.institutionCount}</h2>
                    </div>
                </Link>
                <Link to={StudentsExists ? "/main/Student" : "#"} className="stat-item">
                    <div>
                        <p>Students</p>
                        <h2>{stats.studentCount}</h2>
                    </div>
                </Link>
                <Link to={CoursesExists ? "/main/Course" :"#"} className="stat-item">
                    <div>
                        <p>Courses</p>
                        <h2>{stats.courseCount}</h2>
                    </div>
                </Link>
                <Link to={SubjectsExists ? "/main/Subject" : "#"} className="stat-item">
                    <div>
                        <p>Subjects</p>
                        <h2>{stats.subjectCount}</h2>
                    </div>
                </Link>
                <Link to={DepartmentExists ? "/main/Department" :"#"} className="stat-item">
                    <div>
                        <p>Departments</p>
                        <h2>{stats.departmentCount}</h2>
                    </div>
                 ...
                 userName === 'student' ? <>
                    <main className="main-content">
                        <section className="stats stats12">
                            <Link to={stats1.Student_Profile === 100 ? "/main/Chat" : ""} className="stat-item">
                                <div >
                                    <p>Starred Chat</p>
                                    <h2>{student.chatHistory}</h2>
                                </div>
                            </Link>
                            <Link to={stats1.Student_Profile === 100 ? "/main/Chat" : ""} className="stat-item stats3">
                                <div  >
                                    <p>Chat</p>
                        ...
                        <section className="piecharts">
                            ...
                            <div className='chart'>
                                <div style={{ marginBottom: "20px" }} >
                                    ...
                                    >
                                        Student profile completion</span><br />
                                </div>
                                ...
                                <span className='chart_content'>
                                    ...
                                    Profile completion {stats1?.Student_Profile} %</span>
                            </div>
                        </section>
                    </main>
                </>
